...
- Distribute the whole set as one Claude plugin, `better-accountant-skills`: `.claude-plugin/marketplace.json` makes this repo its own marketplace for Claude Code, and `git archive` builds the zip for Claude and Cowork upload.
...
2026-09-25, Claude Code 2.1.282: `claude plugin validate --strict` passes for the marketplace; the plugin manifest passes with one expected warning (the maintainers' `CLAUDE.md` is not plugin context); and `claude -p --plugin-dir .` loads all eleven skills and both agents under the `better-accountant-skills:` namespace.
