...
`<plugin root>/skills/financial-close/close/SKILL.md`. Paths written `<plugin root>/...` start there. A
...
2. Gather client context from every readable source before asking anything, trusted in
   ...
   choice to the user and wait for it: run the setup-client skill now (it can harvest from
