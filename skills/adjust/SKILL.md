...
`<plugin root>/skills/adjust/SKILL.md`. Paths written `<plugin root>/...` start there. A
...
- Deferred revenue recognition for streams whose invoices match delivery (one
  ...
  the revenue skill's: when the profile has a Revenue section, run the revenue skill's
...
it like any authorized policy, with schedule-shaped arithmetic, an entry under the je
...
4. Completeness scan against the period's activity, both directions: a new payment to a
   ...
   contract routes to the revenue skill instead of a question here: bundles, setup fees,
...
5. Draft each entry under the je skill's contract: balanced, dated within the period,
...
8. Emit the import CSV of approved entries only, per the je skill's CSV contract, and file
...
The entry contract and import CSV format live with the je skill.
