...
`<plugin root>/skills/close-setup/SKILL.md`. Paths written `<plugin root>/...` start there. A
...
3. Earned versus billed: when the business gets paid relative to when it does the work.
   ...
   the revenue skill's Policy setup playbook once this interview is saved, and record the
...
## Revenue
<GAAP contract clients only, written by the revenue skill's Policy setup playbook and
