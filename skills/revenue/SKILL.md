...
`<plugin root>/skills/revenue/SKILL.md`. Paths written `<plugin root>/...` start there. A
...
- **Second review follows the profile.** The profile's `Second review` line decides which
  conclusions go to the `close:rev-challenger` agent before the user sees them. A
...
- **Entries.** Revenue entries go through the je skill's contract and the close's approval
